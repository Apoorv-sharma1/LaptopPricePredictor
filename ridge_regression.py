...
# Scaling features (Crucial for Ridge)
scaler = StandardScaler()
X_train_scaled = scaler.fit_transform(X_train)
X_test_scaled = scaler.transform(X_test)

# Using RidgeCV to automatically find the best alpha
print("Finding optimal alpha for Ridge...")
model = RidgeCV(alphas=np.logspace(-4, 4, 20), cv=5)
model.fit(X_train_scaled, y_train)

print(f"Optimal alpha found: {model.alpha_}")
...
print(f"Reason: This feature has the highest absolute coefficient, meaning it has the strongest impact on the price prediction.")
print(f"\nLeast Important Feature: {least_important['Feature']} (Score: {least_important['Importance']:.2f})")
print(f"Reason: This feature has the smallest coefficient, meaning its impact on the final price is minimal.")
...
plt.title(f"Ridge Regression: Actual vs Predicted (alpha={model.alpha_:.4f})")
plt.show()

# 2. Top 10 Feature Importance
plt.figure(figsize=(12, 6))
top_10 = importance_df.head(10)
plt.barh(top_10['Feature'][::-1], top_10['Importance'][::-1], color='lightgreen')
plt.xlabel("Importance (Absolute Coefficient)")
plt.title("Top 10 Most Influential Features (Ridge)")
plt.show()

# 3. Error Distribution
plt.figure(figsize=(10, 6))
errors = y_test - y_pred
plt.hist(errors, bins=30, edgecolor='black', color='seagreen')
