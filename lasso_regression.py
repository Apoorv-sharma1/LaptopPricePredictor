...
# Scaling features (Crucial for Lasso)
...
model = LassoCV(alphas=np.logspace(-4, 4, 20), cv=5, random_state=42)
...
print(f"Reason: Lasso has reduced this feature's coefficient close to zero, meaning it has little to no influence on the final price.")
...
plt.title(f"Lasso Regression: Actual vs Predicted (alpha={model.alpha_:.4f})")
plt.show()
...
plt.hist(errors, bins=30, edgecolor='black', color='salmon')
